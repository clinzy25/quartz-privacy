...
This section analyzes the various attack vectors associated with smart phones and the ways that they can compromise personal anonymity and privacy, track you, or exploit your personal data. To mitigate these threats, we will configure a new smart phone to be completely anonymous, so that when your activity appears on networks, it is very difficult to associate with your identity.

I will explain the setup briefly so you can decide now if this is something you want to do. Each component will be discussed later in greater detail. You will need to purchase a new, unlocked Google Pixel phone. We will configure it to run a FOSS operating system called GrapheneOS, which is a deGoogled flavor of Android comes with a suite of privacy and security tweaks. We will configure the OS and install apps that improve your PAS (privacy, anonymity and security), as well as some minimal teaks to improve digital well-being.

Once the phone is configured, we will obtain fully anonymized cellular service with Mint Mobile. We will receive a phone number from Mint, but we won't use it for anything. Instead we will configure Voice-over-IP (VoIP) numbers, which is a technology that uses mobile data networks rather than cellular networks to talk and text. This is a far more private method of communication that does not expose your conversations, location, or PII to Mint Mobile or anyone who performs a reverse phone lookup on your cellular number. As a bonus, you get the ability to maintain multiple cellular numbers for different purposes, and the option to easily discard numbers if they become inundated with spam.

Further, we will learn personal OPSEC that prevents us from compromising all of this work. We will explore private alternatives to common applications like maps and messaging, learn to attend to our phone settings on a daily basis to maintain anonymity, and protect our home address. The result is a fully functional smart phone that can download and run most applications, but also one that you can be 100% certain that you are not visible or traceable on any network, and when you need certain functionality like location services, you are visible but not identifiable. This brought to be a surprising sense of peace and relief that I did not expect, as well as some solace that I am not serving Google or Apple with my entire digital life for them to do whatever they want with it.
...
## Phone Network Provider Logging
...
## Non-anonymized Accounts

## Compromised Device

# Threat Response
There are many different flavors of minimal or "dumb" phones that either shed or mask modern network connectivity, often at the cost of functionality, utility and convenience.

# Anonymous Smart Phone
In this guide, we will purchase and configure a Google Pixel phone to run a FOSS Android-based OS called GrapheneOS.
